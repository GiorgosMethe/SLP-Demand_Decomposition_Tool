...
def app_time(x, dfn, dfd, a, b):
	...
	"""
	mean = 0.0
	dist = np.divide(f.pdf(x, dfn, dfd),
	                 (f.cdf(b, dfn, dfd) - f.cdf(a, dfn, dfd)))  # f-dist for duration, truncated from a to b
...
def app_consumption(x, dfn, dfd, a, b):
	...
	"""
	mean = 0.0
	dist = np.divide(f.pdf(x, dfn, dfd, scale=0.1), (
	f.cdf(b, dfn, dfd, scale=0.1) - f.cdf(a, dfn, dfd, scale=0.1)))  # f-dist for duration, truncated from a to b
	dist = np.divide(dist, np.sum(dist))  # normalization
